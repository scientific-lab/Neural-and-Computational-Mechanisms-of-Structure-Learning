...
def updatew(x, y, w_train, w_train1, b_train, b_train1, lr, x_test, cv):
    ...
    y_pred = forw(x_test,w_train,w_train1, b_train, b_train1)#sigmoid_scaled(x_test.dot(w_train))
    loss_pre = np.sum((y_pred - cv)**2)
    
    hidd=sigmoid_scaled(x.dot(w_train))
    grad1 = -2 * hidd.reshape(88, 1) * (y - yy) * sigmoid_derivative_scaled(yy)
    gradb1 = -2*(y - yy) * sigmoid_derivative_scaled(yy)
    #print(grad1.shape)
    #print(w_train1.shape)
    grad=-2 * x.reshape(1024, 1)*((y - yy) * sigmoid_derivative_scaled(yy)).dot(w_train1.T)* sigmoid_derivative_scaled(hidd)
    gradb=-2*((y - yy) * sigmoid_derivative_scaled(yy)).dot(w_train1.T)* sigmoid_derivative_scaled(hidd)
...
## *** 5 Individual Run ***
for iii in range (starting_ind,starting_ind+5):

    error=np.zeros((72,6))
    k=np.zeros((72,6))
    ## ***Generate Stimulus Each Run*** 
    NN=1024
    import numpy as np
    s1=np.random.choice([1,-1],NN)
    s2=s1.copy()
    p2=np.random.choice(range(NN),int(NN/2),replace=False)
    s2[p2]=-s1[p2]
    s3=np.hstack((s1[0:int(NN/2)],s2[0:int(NN/2)]))
    s4=np.hstack((s1[int(NN/2):NN],s2[int(NN/2):NN]))
    p3=np.random.choice(range(NN),int(NN/2),replace=False)
    s3[p3]=-s3[p3]
    p4=np.random.choice(range(NN),int(NN/2),replace=False)
    s4[p4]=-s4[p4]
    s5=np.hstack((s1[0:int(NN/4)],s2[0:int(NN/4)],s3[0:int(NN/4)],s4[0:int(NN/4)]))
    s6=np.hstack((s1[int(NN/4):int(NN/2)],s2[int(NN/4):int(NN/2)],s3[int(NN/4):int(NN/2)],s4[int(NN/4):int(NN/2)]))
    s7=np.hstack((s1[int(NN/2):int(3*NN/4)],s2[int(NN/2):int(3*NN/4)],s3[int(NN/2):int(3*NN/4)],s4[int(NN/2):int(3*NN/4)]))
    s8=np.hstack((s1[int(3*NN/4):NN],s2[int(3*NN/4):NN],s3[int(3*NN/4):NN],s4[int(3*NN/4):NN]))
    p5=np.random.choice(range(NN),int(NN/2),replace=False)
    s5[p5]=-s5[p5]
    p6=np.random.choice(range(NN),int(NN/2),replace=False)
    s6[p6]=-s6[p6]
    p7=np.random.choice(range(NN),int(NN/2),replace=False)
    s7[p7]=-s7[p7]
    p8=np.random.choice(range(NN),int(NN/2),replace=False)
    s8[p8]=-s8[p8]
    ...
    for i in range (1,8):
        m[i]=m[i-1]
        p2=np.random.choice(range(NN),rr,replace=False)
        m[i][p2]=-m[i][p2]
    m[8]=s2
    for i in range (9,17):
        m[i]=m[i-1]
    ...
    for epoch in range(20):
        ...
        for train_index, test_index in kfold.split(X_train,y_train):
            ...
            for x,y in zip(X_train_fold,y_train_fold):
                ...
                w_train,w_train1, b_train, b_train1 = updatew(x, y, w_train,w_train1, b_train, b_train1, lr, X_val_fold, y_val_hippo)
    ...
    ###***Predict OFC Random ***
    predictions=forw(m_sort[np.int32(mm[:,1024])],w_train,w_train1, b_train, b_train1)
    yp_0=predictions
    angle=np.argmax(np.dot(predictions,cv.T),1)*5
    aa=angle-np.arange(72)*5
    aa[aa>180]=360-aa[aa>180]
